...
export default function LocationInput<T extends FieldValues>(props: Props<T>) {
  ...
  useEffect(() => {
    if (field.value && typeof field.value === "object") {
      setInputValue(field.value.location);
    } else {
  ...
  const locationUrl =
    "https://api.locationiq.com/v1/autocomplete?key=pk.613bfadae259820611390e7dbad40a98&limit=5&dedupe=1&";
  ...
  const handleSelect = (location: LocationIQSuggestion) => {
    ...
    field.onChange({  locationName, latitude, longitude });
    setSuggestions([]);
  };
