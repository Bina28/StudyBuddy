...
public class BaseActivtyValidator<T, TDto>: AbstractValidator<T> where TDto: BaseActivityDto
{
  public BaseActivtyValidator(Func<T, TDto> selector)
  {
    ...
    RuleFor(x => selector(x).LocationName).NotEmpty().WithMessage("Location is required");
