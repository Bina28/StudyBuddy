...
public class AccountController(SignInManager<User> signInManager, IEmailSender<User> emailSender, IConfiguration config) : BaseApiController
{
  ...
  public async Task<ActionResult> LoginWithGithub(string code)
  {
    ...
    httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("StudyBuddy");
    ...
    if (string.IsNullOrEmpty(user?.Email))
    {
      ...
      if (emailResponse.IsSuccessStatusCode)
      {
        ...
        if (string.IsNullOrEmpty(primary))
          return BadRequest("Failed to get the email from GitHub");
        user!.Email = primary;
      }
    }
    ...
    if (existingUser == null)
    {
      ...
      var createdResultat = await signInManager.UserManager.CreateAsync(existingUser);
      if (!createdResultat.Succeeded)
  ...
  public async Task<ActionResult> ResendConfirmEmail(string email, string? userId)
  {
    ...
    {
      return BadRequest("Email or userIa must be provided");
    }
